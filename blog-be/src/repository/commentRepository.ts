...
import { commentMapper } from "../utils/commentMapper";
import { pool } from "./connection";

const SELECT_COMMENTS_BY_POST_ID = `SELECT * FROM comments WHERE postId = $1 ORDER BY createdate DESC;`;
...
export const getAllByPost = async (postId: string) => {
  ...
  return results.rows.map(commentMapper);
};
...
export const getById = async (id: string) => {
  ...
  return result.rows.map(commentMapper)[0];
};
